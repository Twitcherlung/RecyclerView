...
class UserService {
    //Локальная переменная для хранения списка пользователей из User
    private var users = mutableListOf<User>()

    //Лок.переменная для складывания всех слушателей которые будут прослушивать все изменения в классе
    private val listeners = mutableSetOf<UsersListener>()

    init {
        val faker = Faker.instance()
        //Перемешаем с помощью метода shuffle
        IMAGES.shuffle()
        users = (1..100).map {
            User(
                id = it.toLong(),
    ...
    fun deleteUser(user: User) {
        //Метод проходит по всему списку
    ...
    //Приватная функция для уведомления слушателей без повторяющегося кода в фунциях выше
    ...
    //Компаньон для списка ссылок на картинки
    companion object {
